<template>
  <create-template>
    <div slot="header" class="create-header">
      <span>{{$t("billing.setting") }}{{$t(`common.cpu`)}}{{ $t("billing.Price")}}</span>
      <help-trigger name="CPUbillingsettings" :style="{'left': '130px', 'top': '30px'}"></help-trigger>
      <span class="create-back" @click="$router.push({name: 'billingsettings'})">
    ...
    <div slot="body" class="create-body">
      <div class="operation-row">
        ...
        <div>{{$t('common.cpu')}}</div>
      </div>
      <div class="operation-row">
        <div class="title requried">{{$t('billingConfig.price')}}</div>
        <input type="text" v-model="price" :class="{'is-error': emptyprice ||invalidprice}"
        :placeholder="$t('billing.billPriceRange')"
        @blur="validate('price')"/>
        <div class="error error-text" v-if="emptyprice && !invalidprice">{{$t('billingConfig.price')}}{{$t('error.noEmpty')}}</div>
        <div class="error error-text" v-if="invalidprice && !emptyprice">{{$t('error.invalid')}}{{$t('billingConfig.price')}}</div>
      </div>
      <div class="operation-row">
    ...
    <div slot="footer" class="create-footer">
      ...
      <span class="btn-cancel" @click="$router.push({name:'billingsettings'})">{{$t('common.cancel')}}</span>
    </div>
  </create-template>
</template>
...
export default {
  ...
  methods: {
    ...
    confirm(){
      ...
      self.$router.push({name: 'billingsettings', params: {resourceName:'cpu'}})
    },
  }
};
</script>
