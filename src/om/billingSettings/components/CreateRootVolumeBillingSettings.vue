<template>
  <create-template>
    <div slot="header" class="create-header">
      <span>{{$t("billing.setting") }}{{$t(`common.rootVolume`)}}{{ $t("billing.Price")}}</span>
      <help-trigger name="RootVolumebillingsettings" :style="{'left': '128px', 'top': '30px'}"></help-trigger>
      <span
        class="create-back"
        @click="$router.push({name: 'billingsettings',params:{resourceName: 'rootVolume'}})"
      >
...
export default {
  name: "CreateRootVolumeBillingSettings",
  mixins: [WindowBase],
  components: {
    CreateTemplate,
    DropDown
  ...
  methods: {
    ...
    confirm() {
      ...
      self.$router.push({
        ...
        params: { resourceName: "rootVolume" }
      });
    }
  }
};
