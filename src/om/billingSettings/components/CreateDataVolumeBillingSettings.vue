<template>
  <create-template>
    <div slot="header" class="create-header">
      <span>{{$t("billing.setting") }}{{$t(`common.dataVolume`)}}{{ $t("billing.Price")}}</span>
      <help-trigger name="dataVolumebillingsettings" :style="{'left': '145px', 'top': '30px'}"></help-trigger>
      <span
        class="create-back"
        @click="$router.push({name: 'billingsettings',params:{resourceName: 'dataVolume'}})"
      >
...
export default {
  name: "CreateDataVolumeBillingSettings",
  mixins: [WindowBase],
  components: {
    CreateTemplate,
    DropDown
  ...
  methods: {
    ...
    confirm() {
      ...
      self.$router.push({
        ...
        params: { resourceName: "dataVolume" }
      });
    }
  }
};
