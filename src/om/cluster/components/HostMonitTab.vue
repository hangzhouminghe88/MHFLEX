    ...
    <div class="chart">
      <detail-monitor
        :param="{
          ...
          name: `${$t('host.NetworkAllInAndOutBytes')}${$t('common.colon')}`,
    ...
    <div class="chart">
      <detail-monitor
        :param="{
          ...
          name: `${$t('host.DiskAllReadAndWriteOps')}${$t('common.colon')}`,
  ...
  export default {
    ...
    methods: {
      ...
      queryCpuAllUsedUtilizationData: function () {
        ...
        let script = `tmp = call("org.zstack.zwatch.api.APIGetMetricDataMsg", '{"namespace": "ZStack/Host", "metricName": "CPUAllUsedUtilization", "period": ${self.cpuAllUsedUtilizationTimeList[timeIndex].step}, "offsetAheadOfCurrentTime": ${self.cpuAllUsedUtilizationTimeList[timeIndex].value}, "labels": ["HostUuid=~ ${self.uuidList.join('|')}"]}')
      ...
      queryMemoryUsedInPercentData: function () {
        ...
        let script = `tmp = call("org.zstack.zwatch.api.APIGetMetricDataMsg", '{"namespace": "ZStack/Host", "metricName": "${metricName}", "period": ${self.cpuAllUsedUtilizationTimeList[timeIndex].step}, "offsetAheadOfCurrentTime": ${self.cpuAllUsedUtilizationTimeList[timeIndex].value}, "labels": ["HostUuid=~ ${self.uuidList.join('|')}"]}')
      put("metricData", tmp.result)
      `
        rpc.query('batch-queries', {
          script: encodeURIComponent(script)
      ...
      queryNetworkData: function () {
        ...
        let script = `tmp = call("org.zstack.zwatch.api.APIGetMetricDataMsg", '{"namespace": "ZStack/Host", "metricName": "${metricName}", "period": ${self.networkTimeIntervalList[timeIndex].step}, "offsetAheadOfCurrentTime": ${self.networkTimeIntervalList[timeIndex].value}, "labels": ["HostUuid=~ ${self.uuidList.join('|')}"]}')
        put("metricData", tmp)
        `
        let p = rpc.query('batch-queries', {
          ...
          .then((resp) => {
            ...
            networkData[0] = {
              ...
              values: _value,
              yAxis: {min: 0, max: maxValue > 5 ? maxValue * 1.1 : 5},
              formataFuc: (it) => { return self.bytesToSizeInMonitor(it, 'B/s', 0) }
            }
            return new Promise((resolve, reject) => { resolve() })
          })
        tasks.push(p)
      ...
      queryDiskData: function () {
        ...
        let script = `tmp = call("org.zstack.zwatch.api.APIGetMetricDataMsg", '{"namespace": "ZStack/Host", "metricName": "${metricName}", "period": ${self.diskTimeIntervalList[timeIndex].step}, "offsetAheadOfCurrentTime": ${self.diskTimeIntervalList[timeIndex].value}, "labels": ["HostUuid=~ ${self.uuidList.join('|')}"]}')
        put("metricData", tmp)
        `
        let p = rpc.query('batch-queries', {
          script: encodeURIComponent(script)
          ...
          .then((resp) => {
            ...
            diskData[0] = {
              ...
              values: _value,
              yAxis: {min: 0, max: maxValue > 5 ? maxValue * 1.1 : 5},
              formataFuc: (it) => { return self.bytesToSizeInMonitor(it, 'ops/s', 0) }
            }
            return new Promise((resolve, reject) => { resolve() })
          })
        tasks.push(p)
      ...
      getAverageValue: function (value, metricName) {
        ...
        for (let key in data) {
          let _value
          if (metricName === 'NetworkAllOutBytes' || metricName === 'NetworkAllInBytes' || metricName === 'DiskAllReadOps' || metricName === 'DiskAllWriteOps') _value = _.sumBy(data[key], function (o) { return o.value })
