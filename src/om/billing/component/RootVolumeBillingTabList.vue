  ...
  export default {
    ...
    computed: {
      volumeItemList: {
        get(){
          let self = this;
          return self.windowData.rootVolumeBillingDetails;
        },
        set(val){
          let self = this;
          self.windowData.rootVolumeBillingDetails = val;
        }
      },
      ...mapGetters({
        get: 'volume/get'
    ...
    methods: {
      ...
      chunkList: function (size = 20) {
        let billingList = _.chunk(this.windowData.rootVolumeBillingDetails, size)
      ...
      init: function () {
        if (this.param) {
          ...
          let rootVolumeBillingDetails = this.sortForLink(this.dbData.billing[dataUuid].rootVolumeBillingDetails, this.isDeleted)
          this.updateWindow({ rootVolumeBillingDetails })
        }
      },
      isDeleted: function (uuid) {
        let result = false
        let volume = this.itemList[uuid]
        result = !(volume && volume.format !== 'vmtx')
        if (volume) {
          result = !!volume.isDestroyed
        }
        return result
        // return _.has(this.dbData.volume, uuid)
      },
      ...
      queryVolume(){
        ...
        self.windowData.currbBillingList.forEach((item) => {
          return p = self.dispatchAction('volume/query', item.resourceUuid);
        })
        task.push(p);
