  ...
  export default {
    methods: {
      create: async function (param) {
        ...
        if (param.applicationType === 'email') {
          ...
          let p = {
            ...
            description: param.description,
            url: param.url,
            atPersonPhoneNumbers: param.atPersonPhoneNumbers.length > 0 ? param.atPersonPhoneNumbers : null,
            atAll: param.atAll
          }
      ...
      isSystemEndpointUuid: function (uuidList) {
        ...
        return uuidList.some(uuid => systemUuid === uuid)
      },
      createSystemEndpoint: async function () {
        ...
        let systemEndpoint = await rpc.create('sns/application-endpoints/http', {
          name: 'system-endpoint',
          description: 'endpoint for reporting system defined alarms',
          url: 'http://system-endpoint.XXX_XXX.default'
        })
        if (!systemEndpoint.inventory) return
        let systemEndpointUuid = systemEndpoint.inventory.uuid
        try {
          await rpc.post(`sns/topics/${systemTopicUuid}/endpoints/${systemEndpointUuid}`, null)
          await this.updateDbObject({
            name: 'common',
            data: {
              systemEndpointUuid: systemEndpointUuid
            }
          })
        } catch (err) {
          console.log(err)
      ...
      batchTaskWrapper: function (p = {
        ...
        const self = this
        let event = self.createEvent(p.eventName, p.uuidList.length === 1 ? self.dbData.zwatchEndpoint[p.uuidList[0]] && self.dbData.zwatchEndpoint[p.uuidList[0]].name : '', p.uuidList.length)
        let tasks = []
