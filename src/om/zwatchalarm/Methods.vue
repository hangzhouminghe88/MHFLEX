<script>
  ...
  import EndPointMethods from 'src/om/zwatchEndPoint/Methods'
    ...
    methods: {
      ...
      getSystemALarmName(name) {
        var obj = {
          ...
          'ZStack Primary Storage Physical Available Capacity Alarm': this.$t('zwatchAlarm.primaryStoragePhysicalAvailableCapacity'),
          ...
          'ZStack Host Root FileSystem Used Capacity Alarm': this.$t('zwatchAlarm.hostRootFileSystemUsedCapacityAlarm'),
          'Volume Xfs Fragmentation Alarm': this.$t('zwatchAlarm.VolumeXfsFragCount'),
          'Host Xfs Fragmentation Alarm': this.$t('zwatchAlarm.DiskXfsFragInPercent')
        };
        return obj[name] || name;
      },
      getAlarmName(uuid, currTab) {
        let self = this;
        if (!currTab) currTab = self.windowData.currTab;
        if (!self.dbData[currTab][uuid]) return;
        var systemAlarmUuidList = self.getZWatchSystemAlarmUuidList();
        var isSystemAlarm = systemAlarmUuidList.some((item) => {
          return item === uuid;
        });
        if (currTab === 'zwatchResourceAlarm') {
          var name = self.dbData[currTab][uuid].name;
          if (isSystemAlarm) {
        ...
        if (currTab === 'zwatchEventAlarm') {
          var eventName = self.dbData[currTab][uuid].eventName;

          return self.$t('zwatchAlarm.eventName.' + eventName, { name: '' });
        }
      },
      ...
      getResourceType: function (namespace, uuid) {
        let key = namespace
        if (this.dbData.zwatchResourceAlarmA[uuid] && this.dbData.zwatchResourceAlarmA[uuid].tag === 'VRouter') {
          key = 'ZStack/VRouter'
        }
        let types = {
          'ZStack/Image': 'image',
