       ...
       <scroll-bar>
         <div class="normal">
           <div class="flex-body" style="padding-top: 0;">
             <div class="left-item">
               <project-user class="project-user" :param="{data: projectAdmin}"></project-user>
...
<script>
  ...
  import MetircData from 'src/component/metric/LineMetric'
  ...
  import ProjectAdminPage from 'src/ecs/home/component/ProjectAdminPage'
  ...
  export default {
    ...
    components: {
      ...
      'project-admin-page': ProjectAdminPage
    },
    created: function () {
      this.updateWindow({
        methods: {
    ...
    mounted: function () {
      ...
      if (localStorage.getItem('loginType') === 'account') {
        ...
        this.identityUuid = localStorage.getItem('userUuid')
        this.isUser = true
    ...
    methods: {
      ...
      tmp = query("QueryIAM2VirtualID projects.uuid=${this.dbData.common.currProject.uuid} count=true")
          ...
          if (resp.inventories.length > 0) {
            ...
            resp.inventories.forEach((item) => {
              item.attributes.forEach((it) => {
                if (item.uuid === self.identityUuid && it.name === 'fullname') {
                  userName = it.value
                }
...
<style scoped>
  ...
  .admin .left-item .bar-ratio-list-top {
    width: 100%;
    height: 260px;
    background: #fff;
    margin-bottom: 20px;
  ...
  .admin .left-item .bar-ratio-list-bottom {
    width: 100%;
    height: 246px;
    background: #fff;
    margin-bottom: 30px;
  ...
  .admin .center-item .item .circle-ratio {
    flex: 1 1 auto;
  ...
  .admin .center-item .bar-ratio-list {
    width: calc(100% - 20px);
    height: 526px;
    background: #fff;
    margin-bottom: 30px;
  ...
  .admin .right-item .bar-ratio-list {
    width: calc(100% - 20px);
    height: 526px;
    background: #ffffff;
    margin-bottom: 30px;
  ...
  .admin .isOpensource .left-item .bar-ratio-list-top,
  .admin .isOpensource .center-item .bar-ratio-list,
  .admin .isOpensource .right-item .bar-ratio-list {
    height: 326px;
  }
