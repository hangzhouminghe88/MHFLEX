<template>
  <div class="container">
    <div class="body">
      <div v-permission="'VOLUME.SNAPSHOT.CREATE'" class="snapshot tree" :style="{ width: (yMax + 1) * ySpace + 'px' }">
        <div class="fake-node" style="display: inline-block;" v-permission="'VOLUME_VOLUME-SNAPSHOT.CREATE'" :class="{'disabled-text': !canCreateSnapshot()}" @click="canCreateSnapshot() && openCreateSnapshot()" :title="$t('common.createSnapshot')">
          <i class="el-icon-circle-plus" style="font-size: 22px;color:#409EFF;" :style="{'color': !canCreateSnapshot() ? '' :  '#409EFF'}"></i>
        </div>
        <help-trigger name="createSnapshot" :style="{ position: 'absolute', top: '9px', left: '45px' }" />
      </div>
      <div class="list" :style="{ 'margin-left': (yMax + 1) * ySpace + 'px' }" :class="{'list-left': snapshotList.length === 0}">
        <div class="row-title">
          <span id="common-name" class="name">{{$t("common.name")}}</span>
          <span id="common-size" v-if="primaryStorageType !== 'Ceph'" class="size">{{$t("common.size")}}</span>
          <span id="common-createDate" class="date">{{$t("common.createDate")}}</span>
        </div>
        <div class="row" v-for="it in snapshotList" v-if="!it.data.fakeLeaf">
          <span id="vm-snapshot-root" class="root-name" v-if="it.data.root">{{ $t('vm.snapshot.root') }}</span>
          <span class="name" v-if="!it.data.root" :title="it.data.inventory.name" @click="goToDetail({ uuid: it.data.inventory.uuid, volumeUuid: it.data.inventory.volumeUuid, volumeType: it.data.inventory.volumeType, refresh: query, primaryStorageType })">{{it.data.inventory.name}}</span>
          <span class="size" v-if="!it.data.root && primaryStorageType !== 'Ceph'">{{it.data.inventory.size && bytesToSize(it.data.inventory.size)}}</span>
          <span class="date" v-if="!it.data.root">{{formatDatetime(new Date(it.data.inventory.createDate))}}</span>
        </div>
      </div>
      <create-snap-dlg v-if="showCreateSnapDlg" :model="showCreateSnapDlg" :message="createSnapMessage" @close="closeCreateSnapDlg"></create-snap-dlg>
    </div>
  </div>
</template>
        ...
        rpc.query('volume-snapshots/trees', {
          ...
          .then((resp) => {
            ...
            let fakeRoot = {
              ...
              children: resp.inventories.map(it => { return it.tree }),
              root: true
            }
