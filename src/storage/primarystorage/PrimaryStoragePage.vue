<template>
  <page-template>
    <div slot="page-header" style="height: 60px; line-height: 60px;">
      <el-row :gutter="10">
        <el-col :span="2">
          <span class="page-header-title">{{ $t(`common.primarystorage`)}}</span>
        </el-col>
        <el-col :span="2.2">
    ...
    <div slot="page-toolbar" class="page-toolbar-container">
      <el-row type="flex" justify="space-between">
        <div style="flex-shrink: 0;">
          <span @click="updateWindow({ currItemUuid: '' });openCreatePrimaryStorage()" class="btn-success"><i class="el-icon-plus icon"></i>{{$t('common.addPrimaryStorage')}}</span>
          <span class="btn-primary" :class="{ 'disabled': !isSelected || instates('Enabled') }" @click="isSelected && !instates('Enabled') && pageEnable()"><i
            class="el-icon-caret-right icon"></i>{{$t('common.enable')}}</span>
          <span class="btn-primary" :class="{ 'disabled': !isSelected || instates('Disabled')}" @click="isSelected && !instates('Disabled') && pageDisable()"><i
            ...
            <span slot="content">
              <transition name="fade" type="transition" mode="out-in">
                <div class="dropdown-content" style="padding: 4px 20px;">
                  <a id="common-reconnect" @click="canReconnect() && pageReconnect()" :class="{'disabled-text': !canReconnect()}" style="padding-top: 12px;">{{ $t("common.reconnect") }}</a>
                  <a id="common-createVolume" @click="canCreateVolume() && pageCreateVolume()" :class="{ 'disabled-text': !canCreateVolume()}">{{ $t("common.createVolume") }}</a>
                  <a id="common-attachCluster" @click="canAttach() && pageAttachCluster()" :class="{ 'disabled-text': !canAttach() }">{{ $t("common.attachCluster") }}</a>
                  <a id="common-detachCluster" @click="canDetachCluster() && pageDetachCluster()" :class="{ 'disabled-text': !canDetachCluster() }">{{ $t("common.detachCluster") }}</a>
                  <a id="common-intoMaintain" @click="canMaintenance() && pageMaintain()" :class="{'disabled-text': !canMaintenance() }">{{ $t("common.intoMaintain") }}</a>
                  <a id="common-destroy" @click="canDelete() && pageDelete()" :class="{'disabled-text': !canDelete()}" style="padding-bottom: 12px;">{{ $t("common.destroy") }}</a>
                </div>
              </transition>
            </span>
          </drop-down>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <a class="link" @click="$router.push(`detailprimarystorage/${scope.row.uuid}`);">{{scope.row.name}}</a>
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('common.type')"
        ...
        >
          <template slot-scope="scope">
            <table-body-item-state slot="item" :state="dbData.primarystorage[scope.row.uuid].state"/>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <table-body-item-state slot="item" :state="dbData.primarystorage[scope.row.uuid].status"/>
          </template>
...
<script>
  ...
  export default {
      ...
      mixins: [Root,WindowBase,PrimaryStorageList, MultiSelectList, PageBase],
      ...
      methods: {
        ...
        pageAttachCluster: async function () {
          ...
          const self = this
          let uuidList = [], selectedList = []
          uuidList = await primaryStorageConditon.getPrimaryStorageAttachableCluster(this.dbData.primarystorage[this.selectedList[0]], this.dbData.common.isOpensource)
          selectedList = self.selectedList[0];
          self.openDialog('ClusterSelectListDlg', {
            primaryStorageUuid: self.selectedList[0],
            conditions: [`uuid?=${uuidList}`],
            type:'selection',
            select: (clusterUuidList) => {
              self.attachDataClustertoPrimaryStorage(clusterUuidList, selectedList).then(() => self.queryList())
            }
          })
        },
        pageDetachCluster: function () {
          const self = this
          let selectedList = self.selectedList[0];

          self.openDialog('ClusterSelectListDlg', {
            primaryStorageUuid: self.selectedList[0],
            conditions: [`primaryStorage.uuid=${self.selectedList[0]}`],
            type:'selection',
            select: (clusterUuidList) => {
              self.detachDataClustertoPrimaryStorage(clusterUuidList, selectedList).then(() => self.queryList())
            }
          })
