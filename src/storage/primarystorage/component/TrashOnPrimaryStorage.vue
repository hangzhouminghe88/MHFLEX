...
<script>
  ...
  export default {
    ...
    created() {
      let self = this
      let primaryStorageUuid = this.param.uuid
      self.updateWindow({
        pageIndex: 1,
        pageCount: 1,
        pageSize: 20,
    ...
    methods:{
      ...
      getTrashList () {
        return rpc.query('primary-storage/trash', {
          uuid: this.windowData.primaryStorageUuid
        })
          .then((resp) => {
      ...
      queryList () {
        let conditions = _.cloneDeep(this.windowData.originalStr)
              ...
              return self.cleanUp(itemList)
                ...
                .then(() => {
                  self.openDialog('ClearBsMessageDlg', {
                    ...
                    residualSize: self.dbData.primarystorage[self.windowData.primaryStorageUuid].availableCapacity,
                    ok: () => {
      ...
      cleanUp (itemList) {
        ...
        itemList.forEach(function (uuid) {
          ((item) => {
            let p = rpc.put(`primary-storage/${self.windowData.primaryStorageUuid}/trash/actions`, {
    ...
    watch: {
      'param': function (newValue, oldValue) {
        ...
        this.updateWindow({
          primaryStorageUuid: this.param && this.param.uuid ? this.param.uuid : ''
