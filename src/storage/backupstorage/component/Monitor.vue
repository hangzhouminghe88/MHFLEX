  ...
  <div class="container">
    <div class="body" @scroll="onScroll">
      <detail-monitor :param="{
        name: $t('backupStorage.UsedCapacityInPercent')+$t('common.colon'),
        title: $t('common.' + usedCapacityTimeList[windowData.usedCapacityIntervalIndex].name),
        timeIntervalList: usedCapacityTimeList,
        hasValue: !isNoDataForUsedCapacity,
        changeInterval: changeInterval,
        type: 'usedCapacity',
        data: windowData.usedCapacityData
      }"></detail-monitor>
    </div>
...
export default {
  ...
  data: function () {
    ...
    return {
      ...
      usedCapacityTimeList: _.cloneDeep(timeIntervalList),
  ...
  methods: {
    ...
    query: function () {
      this.queryUsedCapacityData()
    },
    queryUsedCapacityData: function () {
      let self = this
      let index = self.windowData.usedCapacityIndex
      let timeIndex = self.windowData.usedCapacityIntervalIndex
      let metricName = self.usedCapacityList[index].metricName
      let usedCapacityData = []
      let script = `tmp = call("org.zstack.zwatch.api.APIGetMetricDataMsg", '{"namespace": "ZStack/BackupStorage", "metricName": "${metricName}", "period": ${self.usedCapacityTimeList[timeIndex].step}, "offsetAheadOfCurrentTime": ${self.usedCapacityTimeList[timeIndex].value}, "labels": ["BackupStorageUuid=~ ${self.windowData.dataUuid}"]}')
      ...
      rpc.query('batch-queries', {
        ...
        let _value = []
        self.isNoDataForUsedCapacity = false
        ...
        usedCapacityData[0] = {
          ...
          formataFuc: self.formateValue(self.usedCapacityList[index].unit)
        }
        self.updateWindow({ usedCapacityData })
      }, () => {
        self.isNoDataForUsedCapacity = true
        self.updateWindow({ usedCapacityData: [] })
      })
    },
