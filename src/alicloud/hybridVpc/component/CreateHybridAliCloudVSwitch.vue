  ...
  <create-template-no-route>
    ...
    <div slot="body" class="create-body">
      <div class="operation-row">
        <div class="title required">{{$t('common.hybrididentityzone')}}</div>
        <add-or-delete-input :value="dbData.hybridIdentityZone[identityZoneUuid]
                             && dbData.hybridIdentityZone[identityZoneUuid].zoneName"
                             @open="openHybridIdentityZoneSelect"
                             :class="{'is-error': emptyidentityZoneUuid}"/>
        <div class="error error-text" v-if="emptyidentityZoneUuid">
          {{$t('error.emptyInput') + $t('common.hybrididentityzone')}}
        </div>
      </div>
      <div class="operation-row">
...
<script>
  ...
  export default {
    ...
    created() {
      let self = this;
      if (self.param) {
        if (self.param.vpcUuid) {
          self.vpcUuid = self.param.vpcUuid
    ...
    methods: {
      ...
      initHbridVirtualPrivateCloud () {
        ...
        if (this.identityZoneUuid) {
          identityZone = _.cloneDeep(this.dbData.hybridIdentityZone[this.identityZoneUuid])
      ...
      openHybridIdentityZoneSelect() {
        ...
        self.openDialog('HybridAliCloudIdentityZoneSingleSelect', {
          conditions: [],
          select: (uuid) => self.selectIdentityZone(uuid)
        })
      },

      selectIdentityZone(uuid)  {
        let self = this;
