<template>
  <div class="container">
    <div class="operation-row">
      <div class="title"></div>
      <el-radio-group v-model="sync" @change="handleSync">
        <el-radio :label="true">{{$t("hybriddatacenter.PublicKey")}}</el-radio>
        <el-radio :label="false">{{ $t("hybriddatacenter.PrivateKey") }}</el-radio>
      </el-radio-group>
    </div>
    ...
    <div class="operation-row" v-if="!sync">
      <div class="title required">{{$t('common.type')}}</div>
      <el-select v-model="type" style="width: 100%" @change="setType">
        <el-option value="AliyunEBS">AliyunEBS</el-option>
        <el-option value="AliyunNAS">AliyunNAS</el-option>
      </el-select>
    </div>
