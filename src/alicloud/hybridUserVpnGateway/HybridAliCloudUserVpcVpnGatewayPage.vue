<template>
  <page-template>
    <div slot="page-header">
      <el-row :gutter="10">
        <el-col :span="2.2">
          <span class="page-header-title">{{$t('common.hybridvpcuservpngateway')}}</span>
        </el-col>
        <el-col :span="2">
      ...
      <el-row type="flex">
        <div class="page-toolbar-left" style="position: relative">
         <span class="btn-success" @click="goToCreate()">
          ...
          <span class="text">{{$t('hybridvpcuservpngateway.create')}}</span>
        </span>
          ...
          <help-trigger name="vpnGateway" :style="{'left': '260px', 'top': '10px'}"/>
        </div>
...
<script>
  ...
  import HybridUserVpnGatewayList from './List';
  ...
  export default {
    ...
    mixins: [MultiSelectList, WindowBase, PageBase, HybridUserVpnGatewayList],
          ...
          fields: [
            {
              ...
              onClick: (item) => {
                self.$router.push({name:'detailHybridAliCloudVpcUserVpnGateway', params: {uuid:item.uuid}})
              },
              className: 'link',
              field: 'name'
            },
    ...
    methods: {
      ...
      goToCreate() {
        let self = this;
        self.$router.push({name: 'createHybridAliCloudVpcUserVpnGateway'});
      }
    },
  }
</script>
