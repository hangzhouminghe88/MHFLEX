...
export default {
  ...
  computed: {
    ...mapGetters({
      get: 'hybridAliCloudVirtualrouterEntry/getList'
    })
  },
  ...
  methods: {
    ...
    queryList() {
      ...
      return self.dispatchAction('hybridAliCloudVirtualrouterEntry/basicQuery', {
    ...
    pageDelete(type) {
      ...
      self.openDialog('ConfirmDlg', {
        ...
        getName: (uuid) => {
          return  self.dbData.hybridvirtualrouterentry[uuid].destinationCidrBlock
        },
        ok: () => {
          self.delete(uuidList, type).then(() => self.queryList())
        }
      })
    },
    ...
    canBatchDeleteEntry() {
      ...
      let canBatch = self.selectedList.every((uuid) => self.dbData.hybridvirtualrouterentry[uuid].type === 'System');
      return !canBatch;
    }
  }
}
