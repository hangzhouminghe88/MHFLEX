...
<script>
  import HybridTencentVirtualRouterList from 'src/tencent/vpcVrouter/List';
  ...
  export default {
    name: "HybridTencentVirtualRouteTab",

    mixins: [MultiSelectList, PageBase, WindowBase, HybridTencentVirtualRouterList],
          ...
          fields: [
            {
              ...
              sortable: true,
              onClick: (item) => {
                self.param.goToTencentVRouter({uuid: item.uuid,refresh: self.queryList});
              },
              className: 'link',
              tooltip: true
            },
    ...
    methods: {
      ...
      getField(field, item) {
        let normalProps = ['name', 'vpcName', 'vrId'];
