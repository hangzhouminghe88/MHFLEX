...
export default {
        ...
        fields: [
          {
            ...
            className: "link",
            onClick: item => {
              self.param.goToSubNetDetail({ uuid: item.uuid, refresh: self.queryList});
            },
            tooltip: true
          },
          {
  ...
  methods: {
    ...
    pageCreate() {
      let self = this, identityZoneUuid = "",
      vpcUuid = "",  param = {
        ok: param => {
      ...
      if(self.$route.name.indexOf('TencentIdentityZone') > -1) {
        identityZoneUuid = self.windowData.dataUuid;
        param['identityZoneUuid'] = identityZoneUuid;
      }else {
        vpcUuid = self.windowData.dataUuid;
        param['vpcUuid'] = vpcUuid;
      }
      self.param.pageCreateVSwitch(param);
    }
  }
};
</script>