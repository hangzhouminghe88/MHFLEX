       ...
       <span class="btn-success" @click="goToCreate()">
         ...
         <span slot="title">
           ...
           <div class="dropdown-content">
              <a style="margin-top: 12px;" @click="isSingleSelected && !dbData.hybridTencentDisk[selectedList[0]].ecsInstanceUuid && pageTencentAttach()" :class="{'disabled-text': !(isSingleSelected && !dbData.hybridTencentDisk[selectedList[0]].ecsInstanceUuid)}">{{$t('common.attach')}}</a>
              <a @click="isSelected && !canDetachEcsInstance() && pageTencentDetach()" :class="{'disabled-text': !(isSelected && !canDetachEcsInstance())}">{{$t('common.detach')}}</a>
              <!--<a @click="isSelected && pageTencentDiskAsync()" :class="{'disabled-text': !isSelected}">从可用区同步</a> -->
              <a style="margin-bottom: 12px;" @click="canDeleteDisk() && pageTencentDelete()" :class="{'disabled-text': !canDeleteDisk()}">{{$t('common.destroy')}}</a>
           </div>
         </span>
          </drop-down>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goToCreate() {
        let self = this;
        self.$router.push({name: 'createHybridTencentDisk'});
      }
    }
  }
</script>
