<template>
	<page-template>
		<div slot="page-header">
			<el-row :gutter="10">
				<el-col :span="2.2">{{$t('common.hybridvpcuservpngateway')}}</el-col>
				<el-col :span="2.2">
        ...
        <div class="page-toolbar-left" style="position: relative">
         <span class="btn-success" @click="goToCreate()">
          ...
          <help-trigger name="vpnTencentGateway" :style="{'left': '260px', 'top': '10px'}"/>
        </div>
...
export default {
	 name: 'HybridTencentVpnUserGateWayPage',

	 mixins: [WindowBase,  PageBase, MultiSelectList, List],
	 ...
	 data() {
          ...
          fields: [
            {
              ...
              onClick: (item) => {
                self.$router.push({name:'detailHybridTencentVpnUsergateWay', params: {uuid:item.uuid}})
              },
              className: 'link',
              field: 'name'
            },
      ...
      self.updateWindow({
        ...
        if(field === 'createDate') return formatDatetime(new Date(item[field]));
			...
			goToCreate() {
				let self = this;
				self.$router.push({name: 'createHybridTencentVpnUsergateWay'})
			}
		}
}
</script>
