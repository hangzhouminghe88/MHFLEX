...
export default {
	name: "HybridTencentInstanceTypeList",

	methods: {
		queryList: function () {
			let self = this;
			self.windowData.loading = true;
			rpc.query('hybrid/tencent/ecs/type', {
				q: this.getCondition(),
				...
				this.updateDbTable({
					tableName: 'HybridTencentEcsInstanceType',
					list: resp.types
				})
				this.updateWindow({
		...
		getData() {
			...
			return self.windowData.uuidList.map((uuid) => {
				return self.dbData.HybridTencentEcsInstanceType[uuid]
			})
		}
	}
}