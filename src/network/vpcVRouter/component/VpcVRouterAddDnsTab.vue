  ...
  <div class="container">
    ...
    <div slot="page-table-content" style="max-height: 400px;">
      <el-table :data="windowData.uuidList" @selection-change="handleSelect">
        ...
        <el-table-column :label="$t('common.createDate')" show-overflow-tooltip>
          <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      queryList: function () {
        const self = this
        rpc.query(`vpc/virtual-routers/${self.windowData.dataUuid}`)
          .then((resp) => {
      ...
      pageAddDns: function () {
        this.openDialog('AddDNSDlg', {
          ok: (dns) => this.addDnsToVpcVRouter(this.windowData.dataUuid, dns)
        })
      },
      addDnsToVpcVRouter: function (vpcVRouterUuid, dns) {
        let event = this.createEvent('vpcVRouter.action.addDns',  this.dbData.vm[vpcVRouterUuid].name)
        let self = this
        rpc.create(`vpc/virtual-routers/${vpcVRouterUuid}/dns`, {
          'dns': dns
        }, event).then((resp) => {
          self.param.init();
          self.queryList();
      ...
      pageDeleteDns: function (vpcVRouterUuid) {
        let self = this
        let uuidList = []
        uuidList = self.selectList;
        this.openDialog('ConfirmDlg', {
          title:'vpcVRouter.deleteDns',
          description: 'vpcVRouter.info.deleteDns',
          icon: 'dns',
          uuidList,
          ok: () => {
            self.removeDnsFromL3Network(vpcVRouterUuid, uuidList, self.queryList)
          },
          getName: (dns) => {
            return dns.dns;
          }
      ...
      removeDnsFromL3Network: function (vpcVRouterUuid, dnsList, fn) {
        let event = this.createEvent('vpcVRouter.action.deleteDns', this.dbData.vm[vpcVRouterUuid].name)
        const self = this
        dnsList.forEach(function (dns) {
          ((dns) => {
            rpc._delete(`vpc/virtual-routers/${vpcVRouterUuid}/dns?dns=${dns.dns}`, null, event)
              .then((resp) => {
                if(fn) fn();
                self.param.init();
