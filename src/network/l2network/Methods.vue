  ...
  export default {
    methods: {
      queryList: async function () {
        ...
        this.updateWindow({ loading: false,uuidList, table })
        this.updateDbTable({
          tableName: 'l2network',
          list: resp.inventories
        }).then(() => {
      ...
      getItemList () {
        ...
        return self.windowData.uuidList.map( (uuid) => {
          return self.dbData.l2network[uuid]
        })
      },
        ...
        self.openDialog(dlg,
          {
            ...
            type: 'selection',
            select: (clusterList) => {
             let isBaremetal =  self.dbData.cluster[clusterList[0]] && self.dbData.cluster[clusterList[0]].hypervisorType === 'baremetal';
              let options = {
                title: !isBaremetal ? 'common.detachCluster' : 'common.detachBaremetalCluster',
                description: !isBaremetal ?'cluster.detachCluser' : 'baremetalCluster.detachCluster',
                warning: !isBaremetal ? 'cluster.info.text4' : 'cluster.info.text',
                uuidList: clusterList,
                icon: 'cluster_popupico',
                getName() {
                  return self.dbData.cluster[clusterList[0]].name
                },
                ok(){
                  self.detach(uuid, clusterList)
                    .then(() => {
                      self.query();
                    })
                },
      ...
      openCreateL2Network: function () {
        this.$router.push('createl2network');
      },
