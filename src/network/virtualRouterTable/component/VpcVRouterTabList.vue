<template>
  <div class="container">
    <div class="page-toolbar-container">
      <el-row>
        <el-col :span="14">
          <span class="detail-list-title">{{ $t('common.vpcVRouter') }}{{$t("common.colon")}}</span>
      ...
      <el-table :data="itemList" @selection-change="handleSelection">
        ...
        >
          <template slot-scope="scope">
            <a class="link" @click="$router.push({name:'detailvpcnetwork', params: {uuid: scope.row.uuid}})">{{scope.row.name}}</a>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    created() {
      ...
      this.updateWindow({
        ...
        routeTableUuid: this.param && this.param.uuid
    ...
    methods:{
      ...
      updateVRouterRouteTable: function () {
        let self = this
        let uuid = self.windowData.routeTableUuid
      ...
      pageAttach: function (uuid) {
        ...
        this.getAttachableRouterList().then((virtualRouterVmUuidList) => {
          this.openDialog('RouterMultiSelectList', {
            conditions: [`uuid!?=${virtualRouterVmUuidList.join()}`, 'vmNics.l3Network.networkServices.networkServiceType=VRouterRoute', 'hypervisorType!=ESX', 'applianceVmType=vpcvrouter'],
            showTab: false,
            currTab: 'VRouterRoute',
            select: (uuidList) => {
              self.attach(self.windowData.routeTableUuid, uuidList, self.updateVRouterRouteTable)
            }
          })
        })
      },
      pageDetach: function (uuid) {
        ...
        this.openDialog('ConfirmDlg',{
          ...
          uuidList,
          ok: () => {
            self.detach(self.windowData.routeTableUuid, self.selectedList, self.updateVRouterRouteTable)
          },
          getName: (uuid) => {
