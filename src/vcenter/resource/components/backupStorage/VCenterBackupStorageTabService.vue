<script>
  ...
  export default {
    name: "BackupStorageTabService",
    mixins: [BackupStorageList, MultiSelectList, WindowBase],
    props: {
      param: {
        type: Object,
        default: () => {
    ...
    methods: {
      ...
      getCondition () {
        let conditionList = []
        conditionList.push(`uuid?=${this.backupstorageUuidList.join()}`)
      ...
      getField(field, item){
        ...
        let sizeFields = ['availableCapacity', 'totalCapacity'];
      ...
      init () {
        ...
        let tasks = []
        let p = rpc.query('vcenters/backup-storage', {
          q: `vCenterUuid=${self.param.vCenterUuid}`
        }).then((resp) => {
          backupstorageUuidList = backupstorageUuidList.concat(resp.inventories.map(it => it.uuid))
        })
        tasks.push(p)
        Promise.all(tasks).then(() => {
          self.backupstorageUuidList = backupstorageUuidList
          self.updateWindow({
            pageIndex: 1,
            pageCount: 1,
            pageSize: 20,
