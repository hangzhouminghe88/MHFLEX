<template>
  <dialog-template width="900px">
    <div slot="title" class="modal-plain-header">
      <span class="title" v-if="windowData.networkType[0] === 'system=false'">{{$t("common.selectPrivateNetwork")}}</span>
      <span class="title" v-if="windowData.networkType[0] === 'system=true'">{{$t("common.selectPublicNetwork")}}</span>
      <span class="title" v-if="windowData.networkType[0] !== 'system=true' && windowData.networkType[0] !== 'system=false'">{{$t("common.selectSubNetwork")}}</span>
      ...
      <div style="margin: 30px 20px;">
        ...
        <el-table
          ...
          <el-table-column :label="$t('vrouterroutetable.route')" prop="">
            <template slot-scope="scope">
              {{dbData.l3network[scope.row.uuid] && dbData.l3network[scope.row.uuid].vRouterUuid && dbData.vm[dbData.l3network[scope.row.uuid].vRouterUuid].name}}
            </template>
          </el-table-column>
...
<script>
  ...
  export default {
    name: "SubNetWorkSingleSelectListDlg",
    mixins: [WindowBase, L3NetWorkList],
    created(){
      let self = this;
    ...
    methods: {
      ...
      openL3NetworkDetail: function ($event, uuid) {
        $event.stopPropagation()
        let dlg = 'L3NetworkDetailDlg'
        if (this.dbData.l3network[uuid].type === 'L3VpcNetwork') {
          dlg = 'VpcNetworkDetailDlg'
        }
