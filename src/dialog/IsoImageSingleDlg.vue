    ...
    <el-dialog :visible.async="visiabled" @close="close">
      ...
      <div style="padding:30px;overflow-y: hidden;">
        ...
        <el-table-column
          ...
          prop="cpuNum">
          <template slot-scope="scope">
            {{dbData.image[scope.row.uuid].backupStorageRefs && dbData.image[scope.row.uuid].backupStorageRefs.length > 0
              && dbData.image[scope.row.uuid].backupStorageRefs[0].backupStorageUuid
  ...
  export default {
    ...
    created(){
      ...
      self.updateWindow({
        ...
        .then(()=>{
          let vmInstanceUuid = this.message.vmInstanceUuid;
        return rpc.query(`vm-instances/${vmInstanceUuid}/iso-candidates`)
      })
        .then((resp) => {
          let availableISO = resp.inventories.filter(item => item.status === 'Ready' && item.state === 'Enabled');
          return this.updateWindow({
            conditionList: [`uuid?=${availableISO.map(iso => iso.uuid)}`]
          })
        })
        .then(()=>{
          self.queryList();
    ...
    methods: {
      ...
      confirmDlg(){
        let self = this;
        if(self.message.uuidList) {
          self.$emit('close',{uuid: this.templateRadio, index: this.message.index, uuidList: self.message.uuidList});
        }else{
