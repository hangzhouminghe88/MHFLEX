<template>
  <el-dialog :visible.async="visiabled" @close="close">
    <div slot="title">{{$t("common.selectRootVolumeOffering")}}</div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let isRootVolumeOfferingSelect = ''
      if (this.message && this.message.isRootVolumeOfferingSelect) {
        isRootVolumeOfferingSelect = this.message.isRootVolumeOfferingSelect
      }
      this.updateWindow({
        isRootVolumeOfferingSelect,
        loading: false,
        sortBy: 'createDate',
        sortDirection: '-',
        pageIndex: 1,
