<script>
  ...
  import METRICS from 'src/windows/ZWatchAlarm/Metrics.json'
  ...
  export default {
    ...
    methods: {
      ...
      getAlarmMessageContent: function (message) {
        const self = this
        let type = self.getResourceType(message.namespace)
        ...
        let unit = self.METRICS[message.namespace][message.metricName].unit
        if (unit === 'percent') {
          threshold = threshold + '%'
        }
        if (unit === 'byte') {
        ...
        if (message.alarmStatus === 'OK') {
          ...
          return `${metricName}${comparisonOperator}${threshold}${recovered}`
        }
