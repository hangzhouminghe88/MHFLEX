...
import helpers from "../vm/helpers";
...
export default {
  ...
  getList: (state, getters, rootState) => (param) => {
    ...
    itemList.forEach(item => {
      // networkService
      if (rootState.db.networkServiceType && rootState.db.networkServiceType[item.clusterUuid]) {
        item.clusterName = rootState.db.networkServiceType[item.clusterUuid].name
      }
      // owner
