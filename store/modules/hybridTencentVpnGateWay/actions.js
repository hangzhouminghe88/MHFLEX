...
let tableName = 'hybridTencentVpcVpnGateway';
...
export default {
  basicQuery: batchQueryTemplate(async ({commit, rootState, dispatch}, param) => {
    ...
    dispatch('queryRelationData', ret.inventories);
    return {
      total: ret.total,
      inventories: mainMap
    }
  ...
  sync:  batchActionTemplate(async (ctx, {param, progressCb}) => {
    let uuid = param;
    let ret = await api.sync(uuid, progressCb);
    return ret.inventory;
  }),
}
