...
let tableName  = 'hybridSecurityGroup';
...
export default {
    ...
    let retVpc = await  dispatch('queryVpc', uuidList), retVpcObj = {};
    for(let i  in retVpc ) {
      ((item) => {
        retVpcObj[item.uuid] = item;
      })(retVpc[i])
    }
		let targetObj =  Utils.mergeMap(retVpcObj, mainMap);
	...
	queryVpc:( async ({commit, dispatch}, param) => {
      ...
      uuidList = [param]
		...
		for(let item of uuidList){
			p = await hepler.queryVpc(commit, dispatch, item.uuid, item.ecsVpcUuid);
			tasks.push(p);
		}
